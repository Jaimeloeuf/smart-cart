...
import { useCart, useMisc, useNotif } from "../../../store";
...
const cartItem = cartStore.getItem(props.cartItemID);

const name = ref<string>(cartItem.name);
const quantity = ref<number>(cartItem.quantity);
const selectedUnit = ref<(typeof miscStore.units)[number]>(cartItem.unit);
...
async function updateItem() {
  ...
  router.push({
    ...
    params: { cartItemID: props.cartItemID },
  });

  notifStore.setSnackbar("Cart item updated!");
}
</script>
...
<template>
  <div>
    <div class="mb-6 flex flex-row justify-center text-center">
      <router-link
        :to="{ name: CartItemDetailRoute.name, params: { cartItemID } }"
      >
        Cancel
      </router-link>
      <p class="grow text-xl">Edit Item</p>
    ...
    <div class="mb-6 flex flex-row flex-nowrap overflow-x-scroll">
      <div v-for="(imageLink, i) in cartItem.images" :key="i">
        <div class="relative w-[40vw]">
          <img class="aspect-square w-full object-cover" :src="imageLink" />
          <button
            class="absolute right-0 top-0 bg-black bg-opacity-40 p-2.5 pb-1 pt-0 text-2xl font-extralight text-white"
            @click="cartStore.removeImage(cartItem.id, imageLink)"
          >
            x
          </button>
        </div>
    ...
    <label for="image-upload">
      <label class="mb-2 block font-medium text-primary-dark">Image</label>
